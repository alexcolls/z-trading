...
void run() 
{
    ...
    static bool WKstop = false;
    double Run = 0;
    if( ( !WKstop && dow(0) > 1 && dow(0) < 5 ) || ( dow(0) == 5 && hour(0) < 12 ) )
        Run = 0.5;
    if( !WKstop && Run > 0 && hour(0) >= 7 && hour(0) < 13 )
        Run = 1;
    ...
    if( Run == 0.5 ){
        if( Dir != 0 && TrendSize > 0 ){
            algo("Trend");
            if( Osc < 0.5 && priceClose(0) > WKopen && priceClose(0) > MidWK && priceClose(0) < ZZmid ){
                enterLong(TrendSize,-priceClose(0),MinWK);
                if( TrendLev == 0 )
                    TrendPrice = priceClose(0);
            }
            else if( Osc > -0.5 && priceClose(0) < WKopen && priceClose(0) < MidWK && priceClose(0) > ZZmid ){
                enterShort(TrendSize,-priceClose(0),MaxWK);
                if( TrendLev == 0 )
                    TrendPrice = priceClose(0);
            }
        }
