...
def Oanda_Candles(Symbol,params):
    ...
    df = pd.DataFrame({'DateTime':t,'Open':o,'High':h,'Low':l,'Close':c,'Volume':v})
    df = df.set_index('DateTime')
    return df

api = API(access_token=OA['Token'])
g = GSapi('OandaEurope','Volatility')
...
for i in range(60,40):
    df = Oanda_Candles(Symbols[i],params)
    df['Hour'] = df.index.hour
    df['TrueRange'] = (df['High']-df['Low'])/((df['High']+df['Low'])/2)
    ...
    for col in GScols:
        ...
        if sum(df.index == h) < 3:
            ...
            GSupdate = g.update_acell(cell,stat.mean(df['TrueRange'][h]))
        h += 1
