...
//#define ROUND_LIMIT	// round order limit to half pip (needed for IB)
//#define ASSETLIST	"AssetsGP"
//#define ASSET "DAX-FUT-20171215-FDAX-DTB-EUR"
//#define ASSET "ES-FUT-20180316-ES-GLOBEX"
...
#else
	Lots = slider(1,1,0,MAXLOTS,"Lots",0);
#endif
#ifdef PERCENTSTOP	
...
#ifdef LOG_ACCOUNT
	if(!is(LOOKBACK)) {
		printf("\nN %i H %i  B %s E %s M %s  P %s",
			ifelse(mode(NFA),1,0),Hedge,
		...
		if(Pos != 0)
			printf(" T %.2f",Pos);
		for(open_trades)
			if(TradeIsUnfilled || TradeIsPending)
