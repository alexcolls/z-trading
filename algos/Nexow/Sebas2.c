...
const double entershort = 0.90; // PARAMS TO OPTIMIZE
const double exitshort = 0.5; // PARAMS TO OPTIMIZE
...
void StrategyExecutor( int p, double prevval, double lastval ){	
	...
	if(  year() > pairs[p].year && hour() >= pairs[p].start && hour() < pairs[p].end ){
		if(  pairs[p].dir == 0 && prevval >= entershort && lastval < entershort && lastval > exitshort ){ // Enter Short
			asset(pairs[p].sym1);
			setQuantity(MARGIN);
			enterShort();
			asset(pairs[p].sym2);
		...
		else if( pairs[p].dir == 0 && prevval <= enterlong && lastval > enterlong && lastval < exitlong ){ // Enter Long
			asset(pairs[p].sym1);
			setQuantity(MARGIN);
			enterLong();
			asset(pairs[p].sym2);
