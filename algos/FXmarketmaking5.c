...
void run() 
{
    ...
    if( Run == 1 && TrendSize > 0 ){
        ...
        if( WeeklyDir == 1 && priceLow(0) < ZZmid && priceLow(0) > WKopen ){
            enterLong(TrendSize,-priceLow(0),ZZlo);
            TrendDir = 1;
        }
        else if( WeeklyDir == -1 && priceHigh(0) > ZZmid && priceHigh(0) < WKopen ){
            enterShort(TrendSize,-priceHigh(0),ZZhi);
            TrendDir = -1;
        }
