...
int Signal(double X, double Xf, double Y, double Yf, double Z)
{
	int Sig;
	if( X>Xf+Z && Y<Yf-Z )
	{
...
void run(){
	...
	// Currency Returns // 
		...
		StartDate = 2007;	BarPeriod = 60; BarZone = ET;
		...
		double X = AUD+CAD+CHF+EUR+GBP+HKD+JPY+NZD+SGD+USD+XAG+XAU;
